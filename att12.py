...
conn = psycopg2.connect(database="ivat_production", user="odoo", password="odoo2019")
...
def main1():
    ...
    for x in cls:
        ...
        day = date.strftime("%A")
        date_today1 = False
        print("This is our date", date_today1)
        check_out = date_today1
        name = str(x[2])
        m.append(date)
        m.append(day)
        m.append(name)
    ...
    a1 = []
    while i < len(m):
        a1.append('<tr>   <td>'    + str(m[i]) + ' '  + str(m[i + 1]) +     '</td>' + '<td>'    + str(m[i+2])+  '</td>' + '<td>'    +str(m[i+3])+ '</td>' + '<td>'  +str(m[i+4])+ '</td></tr>')
